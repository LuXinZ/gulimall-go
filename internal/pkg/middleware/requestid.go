...
import (
	...
	"gulimall-product/internal/pkg/global"
)
...
func RequestID() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		requestID := c.Request.Header.Get(global.XRequestIDKey)
		...
		c.Set(global.XRequestIDKey, requestID)

		// 将 RequestID 保存在 HTTP 返回头中，Header 的键为 `X-Request-ID`
		c.Writer.Header().Set(global.XRequestIDKey, requestID)
		c.Next()
	}
}
